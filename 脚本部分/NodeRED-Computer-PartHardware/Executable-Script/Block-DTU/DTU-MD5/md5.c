...
static void MD5_transform(MD5_TRANSFORM_PARAM* param, uint32_t ring, MD5_TRANSORM_FUNC func) {
   UINT a, b, c, d, s, k, i;
...
extern void md5_md5(const unsigned char* data, uint32_t len, unsigned char * md5Out) {
   ...
   //UINT ABCD[4] = { 0x67452301L, 0xef123456, 0xef123458, 0x10325476L };
   UINT ABCD[4] = { 0x67452301L, 0x1f123456L, 0x1f123458L, 0x10325476L };
   //UINT ABCD[4] = { 0x674523L, 40232334L, 0x98badcL, 0x103254L };
   //计算时的数据布局
   //1)原始数据
...
void MD5(const void* ps,char *output) {
   ...
   for (uint32_t i = 0; i < 16; i++) {
       output[io++] = CS[(rs[i] >> 4) & 0xF];
       output[io++] = CS[(rs[i] >> 0) & 0xF];
   }
   output[io++] = 0;
   //printf("%s => %s\n", (char*) ps, ss);
