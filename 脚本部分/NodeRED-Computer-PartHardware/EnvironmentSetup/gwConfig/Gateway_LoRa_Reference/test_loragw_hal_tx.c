...
int main(int argc, char **argv)
{
    ...
    for (cnt_loop = 0; cnt_loop < nb_loop; cnt_loop++) {
        ...
        /* Board reset  REN*/
            if (system("/opt/gwConfig/reset_lgw.sh start") != 0) {
                printf("ERROR: failed to reset SX1302, check your /opt/gwConfig/reset_lgw.sh start script\n");
                exit(EXIT_FAILURE);
            }
        }
        ...
        // --------------------------------------------------------------------------------REN
        // 数据包的负载中加入DFRobot-LoRaWAN字段（payload[9]开始加DFRobot-LoRaWAN，后面的全部填充0）
        char customData[] = "DFRobot";
        int customDataLength = strlen(customData);

        for (int i = 0; i < customDataLength; i++) {
            pkt.payload[i + 9] = customData[i];
        }

        for (int i = customDataLength + 9; i < 255; i++) {
            pkt.payload[i] = 0;
        }
        ...
        if (com_type == LGW_COM_SPI) {
            /* Board reset  REN*/
            if (system("/opt/gwConfig/reset_lgw.sh start stop") != 0) {
                printf("ERROR: failed to reset SX1302, check your /opt/gwConfig/reset_lgw.sh start script\n");
                exit(EXIT_FAILURE);
            }
        }
