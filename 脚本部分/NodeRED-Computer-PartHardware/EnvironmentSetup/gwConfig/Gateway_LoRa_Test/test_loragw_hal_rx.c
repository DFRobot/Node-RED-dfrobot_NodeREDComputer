    ...
    while( (quit_sig != 1) && (exit_sig != 1) )
    {
        ...
        if (com_type == LGW_COM_SPI) {
            ...
            if (system("/opt/gwConfig/reset_lgw.sh start") != 0) {
                printf("ERROR: failed to reset SX1302, check your /opt/gwConfig/reset_lgw.sh script\n");
                exit(EXIT_FAILURE);
            }
        }
        ...
        while (((nb_pkt_crc_ok < nb_loop) || nb_loop == 0) && (quit_sig != 1) && (exit_sig != 1)) {
            ...
            if (nb_pkt == 0) {
                ...
                printf( "\nNb valid packets received: %lu status is 0x01 (%lu)\n", nb_pkt_crc_ok, cnt_loop );
            }
        }
        ...
        if (com_type == LGW_COM_SPI) {
            /* Board reset  REN*/
            if (system("/opt/gwConfig/reset_lgw.sh stop") != 0) {
                printf("ERROR: failed to reset SX1302, check your /opt/gwConfig/reset_lgw.sh script\n");
                exit(EXIT_FAILURE);
            }
        }
