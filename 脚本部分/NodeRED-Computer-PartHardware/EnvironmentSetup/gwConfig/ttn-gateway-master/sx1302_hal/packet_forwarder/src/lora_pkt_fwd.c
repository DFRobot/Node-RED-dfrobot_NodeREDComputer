...
static int sock_mup; /* socket for upstream traffic */
static int sock_mdown; /* socket for downstream traffic */
...
static pthread_mutex_t mx_sockup = PTHREAD_MUTEX_INITIALIZER;
...
int main(int argc, char ** argv)
{
    ...
    //////////////////* look for server address modbus upstream port */
    #if 1
    i = getaddrinfo(serv_addr, "1710", &hints, &result);
    if (i != 0) {
        MSG("ERROR: [up] getaddrinfo on address %s (PORT %s) returned %s\n", serv_addr, "1710", gai_strerror(i));
        exit(EXIT_FAILURE);
    }

    /* try to open socket for upstream traffic */
    for (q=result; q!=NULL; q=q->ai_next) {
        sock_mup = socket(q->ai_family, q->ai_socktype,q->ai_protocol);
        if (sock_mup == -1) continue; /* try next field */
        else break; /* success, get out of loop */
    }
    if (q == NULL) {
        MSG("ERROR: [up] failed to open socket to any of server %s addresses (port %s)\n", serv_addr, "1710");
    ...
    /* connect so we can send/receive packet with the server only */
    i = connect(sock_mup, q->ai_addr, q->ai_addrlen);
    if (i != 0) {
        MSG("ERROR: [up] connect returned %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
    ...
    //////////////* look for server address w/ downstream port */
    i = getaddrinfo(serv_addr, "1711", &hints, &result);
    if (i != 0) {
        MSG("ERROR: [down] getaddrinfo on address %s (port %s) returned %s\n", serv_addr, "1711", gai_strerror(i));
        exit(EXIT_FAILURE);
    }

    /* try to open socket for downstream traffic */
    for (q=result; q!=NULL; q=q->ai_next) {
        sock_mdown = socket(q->ai_family, q->ai_socktype,q->ai_protocol);
        if (sock_mdown == -1) continue; /* try next field */
        else break; /* success, get out of loop */
    }
    if (q == NULL) {
        MSG("ERROR: [down] failed to open socket to any of server %s addresses (port %s)\n", serv_addr, "1711");
    ...
    /* connect so we can send/receive packet with the server only */
    i = connect(sock_mdown, q->ai_addr, q->ai_addrlen);
    if (i != 0) {
        MSG("ERROR: [down] connect returned %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
    ...
    i = pthread_create(&thrid_modbus, NULL, (void * (*)(void *))thread_modbus, NULL);
    if (i != 0) {
        MSG("ERROR: [main] impossible to create modbus thread\n");
        exit(EXIT_FAILURE);
    }
...
void thread_modbus(void) {
    ...
    sleep(2);
    modbus_t *m_modbus = modbus_new_rtu("/dev/ttyUSB0", 115200, 'N', 8 , 1);
    //modbus_t *m_modbus = modbus_new_rtu("/dev/ttyS3", 115200, 'N', 8 , 1);
    ...
    modbus_set_response_timeout(m_modbus, 1/*sec*/, 0/*usec*/);
    modbus_set_slave(m_modbus, 0x50/*slave*/);
    ...
    while (!exit_sig && !quit_sig) {
        ...
        *(uint32_t *)(buff + 8) = net_mac_l;
        buff[12] = 'M';  buff[13] = 'R';
        buff[14] = 'U';  buff[15] = 0;
        buff[16] = 0x50; buff[17] = 0;
        buff[18] = 0;    buff[19] = 0;
        token_h = (uint8_t)rand(); /* random token */
        token_l = (uint8_t)rand(); /* random token */
        buff[1] = token_h;
        buff[2] = token_l;
        buff_index = 20; /* 20-byte header */

        //ret = modbus_read_input_bits(m_modbus, 0/*startAddress*/, 24/*noOfItems*/, dest);//24值得是读取端口数量
        ret = modbus_read_registers(m_modbus, 0x34, 0x01, dest16);
        printf("net_mac_h=%x net_mac_l=%x\n",net_mac_h,net_mac_l);
        MSG("%2x %2x %2x %2x\n",dest[0], dest[1], dest[2], dest[3]);
        MSG("dest=%d mm\n",dest16[0]);

        memcpy((void *)(buff + buff_index), (void *)"{\"rxpk\":[", 9);
        buff_index += 9;

        gettimeofday(&tv,&tz);
        printf("tv_sec:%ld \n",tv.tv_sec);

        j = snprintf(buff + buff_index, 1024-buff_index, "{\"tmst\":%ld,",tv.tv_sec);
        buff_index += j;

        j = snprintf(buff + buff_index, 1024-buff_index, "\"modu\":\"MODBUS_RTU\",\"port\":\"/dev/ttyUSB0\",\"addr\":\"80\",\"baud\":\"115200 8N1\",");
        buff_index += j;

        j = snprintf(buff + buff_index, 1024-buff_index, "\"size\":%d,",dest16[0]>999?4:(dest16[0]>99?3:2));
        buff_index += j;

        j = snprintf(buff + buff_index, 1024-buff_index,"\"data\":\"");
        buff_index += j;
        {
            char tmp[20];
            dest16[0] = 4321;
            sprintf(tmp,"%d",dest16[0]);
            j = bin_to_b64(tmp, strlen(tmp), (char *)(buff + buff_index), 341); /* 255 bytes = 340 chars in b64 + null char */
            buff_index += j;
        }

        j = snprintf(buff + buff_index, 1024-buff_index,"\"}]}");
        buff_index += j;

        MSG("%s\n",buff+20);
        //pthread_mutex_lock(&mx_sockup);
        send(sock_mup, (void *)buff, buff_index, 0);
        //pthread_mutex_unlock(&mx_sockup);
        for (i=0; i<20 && !exit_sig && !quit_sig; i++){
            usleep(50*1000);
        }
    }
